# Sub-Agent Task: Spotify Playlists Tool

> **Priority**: 2 (Can run in parallel with other tool sub-agents)
> **Estimated Complexity**: High (includes migration, many endpoints)
> **Files to Create**: 1
> **Files to Modify**: 1
...
Create a new `spotifyPlaylists` tool that handles all playlist-related Spotify API endpoints. This includes **migrating** existing actions and adding new endpoints.
...
### CREATE: `lib/ai/tools/spotify-playlists.ts`
...
Note: Modifying playlists requires you own them or they are collaborative. All actions work without Premium.`,
    ...
    inputSchema: z.object({
      ...
      playlistId: z
        .string()
        .optional()
        .describe("Playlist ID (required for most actions except get_my_playlists and create_playlist)"),
      playlistName: z
        .string()
        .optional()
        .describe("Name for new playlist (required for create_playlist) or new name for change_details"),
      playlistDescription: z
        .string()
        .optional()
        .describe("Description for playlist (optional for create_playlist and change_details)"),
      isPublic: z
        .boolean()
        .optional()
      ...
      trackUris: z
        .array(z.string())
        .optional()
        .describe("Array of track URIs (spotify:track:xxx) for add_tracks, remove_tracks, or reorder_tracks (replace)"),
      limit: z
        .number()
        .optional()
      ...
      snapshotId: z
        .string()
        .optional()
        .describe("Playlist snapshot ID for remove_tracks or reorder_tracks (ensures consistency)"),
    }),

    execute: async ({ 
      action, playlistId, playlistName, playlistDescription, isPublic,
      trackUris, limit, offset, rangeStart, insertBefore, rangeLength, snapshotId
    }) => {
      ...
      try {
        switch (action) {
          ...
          case "get_playlist_tracks": {
            ...
            const result = await service.getPlaylistTracks(playlistId, limit, offset);
            return {
          ...
          case "change_details": {
            ...
            if (!playlistName && !playlistDescription && isPublic === undefined) {
              return {
                error: "missing_changes",
                message: "At least one of playlistName, playlistDescription, or isPublic is required",
              };
            }
          ...
          case "add_tracks": {
            ...
            const result = await service.addTracksToPlaylist(playlistId, trackUris);
            if (result.error) {
              return {
                action: "add_tracks",
                error: result.error,
          ...
          case "remove_tracks": {
            ...
            const result = await service.removeTracksFromPlaylist(playlistId, trackUris, snapshotId);
            return { 
          ...
          case "reorder_tracks": {
            ...
            if (trackUris && trackUris.length > 0) {
              const result = await service.replacePlaylistTracks(playlistId, trackUris);
              return {
            ...
            if (rangeStart === undefined || insertBefore === undefined) {
              return {
                ...
                message: "For reorder: rangeStart and insertBefore are required. For replace: provide trackUris array.",
              };
            }
            const result = await service.reorderPlaylistTracks(playlistId, rangeStart, insertBefore, rangeLength, snapshotId);
            return {
              action: "reorder_tracks",
              operation: "reorder",
              playlistId,
...
Add/modify these methods in the SpotifyService class:

```typescript
// Modify existing getPlaylists to support pagination
async getPlaylists(limit?: number, offset?: number) {
  const params = new URLSearchParams();
  if (limit) params.set('limit', String(limit));
  if (offset) params.set('offset', String(offset));
  const paramString = params.toString() ? `?${params}` : '?limit=20';
  return this.apiCall<{ items: any[]; total: number; next: string | null }>(`/me/playlists${paramString}`);
}

// Modify existing getPlaylistTracks to support pagination
...
async removeTracksFromPlaylist(
  ...
  trackUris: string[],
  snapshotId?: string
): Promise<{ success?: boolean; snapshotId?: string; error?: string; message?: string }> {
  try {
    const body: any = {
...
1. [ ] `lib/ai/tools/spotify-playlists.ts` created with all 8 actions
2. [ ] 5 new service methods added (`getPlaylist`, `changePlaylistDetails`, `removeTracksFromPlaylist`, `replacePlaylistTracks`, `reorderPlaylistTracks`)
3. [ ] 2 existing methods modified for pagination (`getPlaylists`, `getPlaylistTracks`)
4. [ ] All permission errors handled properly
5. [ ] Reorder supports both replace (via trackUris) and reorder (via rangeStart/insertBefore) modes
...
- Uses existing `SpotifyPlaylist` interface and `createPlaylist`, `addTracksToPlaylist` methods
